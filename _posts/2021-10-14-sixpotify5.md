---
title: "[DE 프로젝트: 음악 추천 챗봇 'Sixpotify'] 5. AWS EMR 스파크(Spark)"
description: "AWS 활용 클라우드 환경 데이터 파이프라인 구축 및 음악 추천 챗봇 프로젝트: AWS EMR을 통해 제플린을 활성화 하고 파이스파크를 이용하여 분석"
categories:
 - Project
tags: [프로젝트, 데이터 엔지니어링, AWS, EMR, 스파크, 제플린]
---
...
- `On-cluster application user interfaces`에서 제플린 접속
...
## 데이터프레임(dataFrame)
- `rdd`로 쪼개서 `map`함수를 통해 쪼개놓은 데이터에 각각 맵핑 시켜주고 선택한다.
- `sqlContext`를 통해 S3에 이전에 저장해 놓았던 데이터를 데이터프레임 형태로 불러와 작업한다.
