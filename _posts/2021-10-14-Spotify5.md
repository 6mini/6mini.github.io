...
## RDD
- 2번째 cell까지는 python에서 진행하던 방식
- 3번째 cell부터 spark의 방식
- spark는 rdd를 기반으로 방대한 데이터를 분산시켜서 mapping한 후 apply해서 얻은 값을 통합을 해서 받는 구조
- sc(spark context)를 통해 parallelize하게 3개의 데이터를 쪼개서 rdd로 나누어 준 결과
...
## DF
- `rdd`로 쪼개서 `map`함수를 통해 쪼개놓은 데이터에 각각 mapping 시켜주고 선택
- `sqlContext`를 통해 S3에 이전에 저장해 놓았던 데이터를 dataFrame형태로 불러와 작업
...
'''
+--------------------+----+---------+----------+--------------------+--------------------+--------------------+--------------------+--------------------+----------+--------------------+
|                  id|name|followers|popularity|                 url|           image_url|                  id|           artist_id|                name|popularity|        external_url|
+--------------------+----+---------+----------+--------------------+--------------------+--------------------+--------------------+--------------------+----------+--------------------+
...
- 해당 값을 바로 시각화할 수 있는 점도 장점 중의 하나
...
SELECT name, followers, track_id
FROM joined
LIMIT 10
```

![image](https://user-images.githubusercontent.com/79494088/137518070-464e2755-56bd-4b26-8d0a-a9899d794e29.png)